...
Algorithm_b::Algorithm_b() : maxSteps(0), wallsSensor(nullptr), dirtSensor(nullptr), batteryMeter(nullptr),
...
void Algorithm_b::setMaxSteps(std::size_t maxSteps) {
    this->maxSteps = maxSteps;
}

void Algorithm_b::setWallsSensor(const WallsSensor &wallsSensor) {
    this->wallsSensor = &wallsSensor;
}

void Algorithm_b::setDirtSensor(const DirtSensor &dirtSensor) {
    this->dirtSensor = &dirtSensor;
}

void Algorithm_b::setBatteryMeter(const BatteryMeter &batteryMeter) {
...
void Algorithm_b::updateCurrPosition(Step step) {
    currPosition = moveInDirection(currPosition, getMatchingDirection(step));
}

void Algorithm_b::relax() {
...
Step Algorithm_b::chooseNeighbor() {
...
Step Algorithm_b::moveTo(std::pair<int, int> targetPosition) {
